...
async fn help_single_letter_alias() {
    ...
    let _banner = meshbbs::bbs::commands::CommandProcessor::new().process(&mut session, "ignored", &mut storage, &cfg).await.unwrap();
    // Now in MainMenu: capture baseline help output
    let base = meshbbs::bbs::commands::CommandProcessor::new().process(&mut session, "HELP", &mut storage, &cfg).await.unwrap();
    for variant in ["help", "HeLp", "H", "h", "?"] {
        let out = meshbbs::bbs::commands::CommandProcessor::new().process(&mut session, variant, &mut storage, &cfg).await.unwrap();
    ...
    let user_base = meshbbs::bbs::commands::CommandProcessor::new().process(&mut session, "HELP", &mut storage, &cfg).await.unwrap();
    for variant in ["help", "HELP", "H", "h", "?"] {
        let out = meshbbs::bbs::commands::CommandProcessor::new().process(&mut session, variant, &mut storage, &cfg).await.unwrap();
