...
async fn help_guest_vs_user() {
    ...
    let guest = meshbbs::bbs::commands::CommandProcessor::new().process(&mut session, "help", &mut storage, &cfg).await.unwrap();
    assert!(guest.contains("REGISTER"), "guest help should mention REGISTER");
    assert!(!guest.contains("DELETE <topic>"), "guest help must not show moderator cmds");

    // Simulate login with level 1
    session.login("alice".into(), 1).await.unwrap();
    let user_help = meshbbs::bbs::commands::CommandProcessor::new().process(&mut session, "help", &mut storage, &cfg).await.unwrap();
...
async fn help_moderator_and_sysop() {
    ...
    let mod_help = meshbbs::bbs::commands::CommandProcessor::new().process(&mut mod_session, "HELP", &mut storage, &cfg).await.unwrap();
    ...
    let sys_help = meshbbs::bbs::commands::CommandProcessor::new().process(&mut sys_session, "?", &mut storage, &cfg).await.unwrap();
