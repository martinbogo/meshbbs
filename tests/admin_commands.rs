...
async fn admin_commands_moderator_access() {
    ...
    let mut alice_session =
        meshbbs::bbs::session::Session::new("alice_session".into(), "alice_node".into());
    ...
    // Test USERS command
    server
        .route_test_text_direct(&alice_node, "USERS")
        .await
        .unwrap();
    ...
    // Test WHO command
    server
        .route_test_text_direct(&alice_node, "WHO")
        .await
        .unwrap();
    let response = server.test_messages().last().unwrap().1.clone();
    ...
    server
        .route_test_text_direct(&alice_node, "USERINFO bob")
        .await
        .unwrap();
    let response = server.test_messages().last().unwrap().1.clone();
    ...
    server
        .route_test_text_direct(&alice_node, "SESSIONS")
        .await
        .unwrap();
    let response = server.test_messages().last().unwrap().1.clone();
    ...
    // Test ADMIN command
    server
        .route_test_text_direct(&alice_node, "ADMIN")
        .await
        .unwrap();
    let response = server.test_messages().last().unwrap().1.clone();
...
async fn admin_commands_user_denied() {
    ...
    let mut charlie_session =
        meshbbs::bbs::session::Session::new("charlie_session".into(), "charlie_node".into());
    ...
    for cmd in admin_commands {
        server
            .route_test_text_direct(&charlie_node, cmd)
            .await
            .unwrap();
        let response = server.test_messages().last().unwrap().1.clone();
        assert!(
            response.contains("Permission denied"),
            "Command '{}' should be denied for regular user",
            cmd
        );
    }
}
...
async fn broadcast_and_kick_commands() {
    ...
    let mut admin_session =
        meshbbs::bbs::session::Session::new("admin_session".into(), "admin_node".into());
    ...
    server
        .route_test_text_direct(&admin_node, "BROADCAST System maintenance in 5 minutes")
        .await
        .unwrap();
    let response = server.test_messages().last().unwrap().1.clone();
    ...
    // Test KICK command
    server
        .route_test_text_direct(&admin_node, "KICK troublemaker")
        .await
        .unwrap();
    let response = server.test_messages().last().unwrap().1.clone();
