...
async fn permission_enforcement_read_post() {
    ...
    server
        .test_register("admin_user", "Password123")
        .await
        .unwrap();
    ...
    server
        .test_store_message("general", "alice", "hi")
        .await
        .unwrap();

    // mods area: alice (lvl1) cannot post
    let err = server
        .test_store_message("mods", "alice", "secret")
        .await
        .err();
    assert!(err.is_some());
    // moderator can post
    server
        .test_store_message("mods", "mod", "notice")
        .await
        .unwrap();

    // ann area post_level 10: moderator cannot post
    let err2 = server
        .test_store_message("ann", "mod", "announcement")
        .await
        .err();
    assert!(err2.is_some());
}
