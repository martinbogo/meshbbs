...
impl TlsConfig {
    ...
    pub async fn from_dashboard_config(config: &AdminDashboardConfig, data_dir: &str) -> Result<Option<Self>> {
        match config.tls_mode.as_str() {
            ...
            "letsencrypt" => {
                ...
                let domain = config.letsencrypt_domain.as_ref()
                    .ok_or_else(|| anyhow!("letsencrypt_domain required for Let's Encrypt mode"))?;
                let email = config.letsencrypt_email.as_ref()
            ...
            "custom" => {
                ...
                let cert_path = config.tls_cert.as_ref()
                    .ok_or_else(|| anyhow!("tls_cert required for custom mode"))?;
                let key_path = config.tls_key.as_ref()
